...
public class A04Main {
	 ...
	 * </pre>
	 *
	 * Note: your value may not match this as it is file dependent.
	 *
	 * @param tree The BST to generate the table from.
	 */
	private static void characterTable(final BST<Character> tree) {

		final ArrayList<CountedValue<Character>> array = tree.inOrder();
		int totalCount = 0;
		...
		for (final CountedValue<Character> value : array) {
			...
			System.out.format("%4s %,8d %7.2f%n", value.getValue(), count, percent);
		}
		return;
	}
	...
	private static void fillTree(final BST<Character> tree, final String string) {
		...
		for (final char letter : string.toCharArray()) {
			final CountedValue<Character> value = new CountedValue<Character>(Character.toUpperCase(letter));
			tree.insert(value);
		}
		return;
	}
	...
	private static void testAVL() {
		...
		System.out.println("Insert value: " + Arrays.toString(testData));
