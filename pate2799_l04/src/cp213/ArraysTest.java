...
public class ArraysTest {
	...
	public static boolean task2() {
		...
		// your code here
		for (int i = 0; i < first.length; i++) {
			first[i] *= 2; // Multiply each element by 2
		}

		for (int i = 0; i < second.length; i++) {
			if (second[i] != first[i]) {
				return false; // Return false if any element in 'second' does not match 'first'
			}
		}

		// return true; - or - return false;
		return true; // since all elements match
	}
	...
	public static boolean task3() {
		...
		// your code here
		for (int value : first) {
			value *= 2; // This won't change the actual array's content
		}

		// Check if any value was doubled
		for (int j : first) {
			if (j % 2 == 0) {
				return true;
			}
	...
	public static Integer[] task5() {
		...
		// Convert the ArrayList to an Integer array and return it
